...
export function SimpleCheckout({ packId, packTitle, packDescription, packPrice }: SimpleCheckoutProps) {
  ...
  const handleCheckout = async () => {
    ...
    try {
      // Create a standard checkout session (fallback to working method)
      ...
      if (data.url) {
        ...
        // Fallback: Try to redirect using sessionId if URL is missing
        console.warn('No URL in response, trying with sessionId:', data.sessionId)
        throw new Error('Checkout session created but no redirect URL provided')
      } else {
